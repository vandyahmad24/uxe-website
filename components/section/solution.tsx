...
export default function Solution() {
  ...
  useEffect(() => {
    const doc = document.querySelectorAll('#solution-content > div');

    const handleActiveSolution = () => {
      setIsActiveSolution(!isActiveSolution)
    }

    doc.forEach(e => {
      e.addEventListener('click', handleActiveSolution)
    })

    return () => {
      doc.forEach(e => {
        e.removeEventListener('click', handleActiveSolution)
      })
    }
  })
  return (
    <div className="bg-[#F4F5F6]">
      <div className="max-w-[1440px] mx-auto p-[max(48px,_min(calc(100vw_*_(64_/_1440)),_64px))_max(20px,_min(calc(100vw_*_(70_/_1440)),_70px))] overflow-hidden bg-[url('/image/solution-background.png')] bg-cover">
        <div className="flex flex-col gap-[45px]">
          ...
          <div id='solution-content' className="transition-all flex max-lg:grid max-lg:grid-cols-1 gap-[20px]">
            {/* <div className="grid grid-cols-[calc(70%_-_10px)_calc(30%_-_10px)] max-lg:grid-cols-1 gap-[20px]"> */}
            <div className="transition-all w-full rounded-[12px] bg-[#365EFF] h-[475px] flex flex-col justify-between overflow-hidden bg-[url('/image/solution-image-01.png')] bg-no-repeat bg-cover">
            ...
            <div className="transition-all w-1/2 max-lg:w-full rounded-[12px] bg-[#365EFF] h-[475px] flex flex-col justify-between overflow-hidden bg-[url('/image/solution-business.png')] bg-contain bg-right bg-no-repeat">
              <div className="flex flex-col items-start gap-[12px] p-[24px]">
              ...
              <div className="flex flex-col items-start gap-[20px] p-[24px]">
                <p className="text-[max(14px,_min(calc(100vw_*_(16_/_1440)),_16px))] text-white leading-[132%] -tracking-[.16px] max-w-md">Centralized Surveillance, Real-Time Analysis, and Remote Access for Unparalleled Protection</p>
                <a href="#" className="block text-[14px] text-white font-medium leading-[132%] -tracking-[.14px] p-[10px_16px] rounded-full bg-[#BEBEBE40] border border-[#CFCFCF40]" style={{
                  border: "1px solid rgba(207, 207, 207, 0.25)",
                  backdropFilter: "blur(2px)"
                }}>Get in touch</a>
              </div>
